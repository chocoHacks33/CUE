...
def _load_roster() -> dict[str, str]:
    ...
    for g in data.get("guests", []):
        gid = g.get("id") or ""
        # Prefer the first alias (usually a short first name), else the full name.
        aliases = g.get("aliases") or []
        display = aliases[0] if aliases else g.get("name") or gid
        out[gid] = str(display)
    return out
...
def plain_reason(rec: DecisionRecord) -> str:  # noqa: PLR0911, PLR0912 -- deliberate rule table
    ...
    if "correction re-cut" in reason:
        if who:
            return f"Correction. Cutting to {who}."
        return "Correction. Cutting to the new target."

    # ---- named / wide / slate through decide() ----------------------------
    ...
    if "role_based" in reason or "fresh identity" in reason:
        if who:
            return f"Cutting to {who}. Just invited up."
        return f"Cutting to {_camera_word(cid)}. Just invited up."
